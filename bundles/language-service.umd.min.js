 ...
 * found in the LICENSE file at https://angular.io/license
 */
function createUrlResolverWithoutPackagePrefix(){return new UrlResolver}function createOfflineCompileUrlResolver(){return new UrlResolver(".")}function getUrlScheme(url){var match=_split(url);return match&&match[_ComponentIndex.Scheme]||""}function _buildFromEncodedParts(opt_scheme,opt_userInfo,opt_domain,opt_port,opt_path,opt_queryData,opt_fragment){var out=[];return null!=opt_scheme&&out.push(opt_scheme+":"),null!=opt_domain&&(out.push("//"),null!=opt_userInfo&&out.push(opt_userInfo+"@"),out.push(opt_domain),null!=opt_port&&out.push(":"+opt_port)),null!=opt_path&&out.push(opt_path),null!=opt_queryData&&out.push("?"+opt_queryData),null!=opt_fragment&&out.push("#"+opt_fragment),out.join("")}function _split(uri){return uri.match(_splitRe)}function _removeDotSegments(path$$1){if("/"==path$$1)return"/";for(var leadingSlash="/"==path$$1[0]?"/":"",trailingSlash="/"===path$$1[path$$1.length-1]?"/":"",segments=path$$1.split("/"),out=[],up=0,pos=0;pos<segments.length;pos++){var segment=segments[pos];switch(segment){case"":case".":break;case"..":out.length>0?out.pop():up++;break;default:out.push(segment)}}if(""==leadingSlash){for(;up-- >0;)out.unshift("..");0===out.length&&out.push(".")}return leadingSlash+out.join("/")+trailingSlash}function _joinAndCanonicalizePath(parts){var path$$1=parts[_ComponentIndex.Path];return path$$1=null==path$$1?"":_removeDotSegments(path$$1),parts[_ComponentIndex.Path]=path$$1,_buildFromEncodedParts(parts[_ComponentIndex.Scheme],parts[_ComponentIndex.UserInfo],parts[_ComponentIndex.Domain],parts[_ComponentIndex.Port],path$$1,parts[_ComponentIndex.QueryData],parts[_ComponentIndex.Fragment])}function _resolveUrl(base,url){var parts=_split(encodeURI(url)),baseParts=_split(base);if(null!=parts[_ComponentIndex.Scheme])return _joinAndCanonicalizePath(parts);parts[_ComponentIndex.Scheme]=baseParts[_ComponentIndex.Scheme];for(var i=_ComponentIndex.Scheme;i<=_ComponentIndex.Port;i++)null==parts[i]&&(parts[i]=baseParts[i]);if("/"==parts[_ComponentIndex.Path][0])return _joinAndCanonicalizePath(parts);var path$$1=baseParts[_ComponentIndex.Path];null==path$$1&&(path$$1="/");var index=path$$1.lastIndexOf("/");return path$$1=path$$1.substring(0,index+1)+parts[_ComponentIndex.Path],parts[_ComponentIndex.Path]=path$$1,_joinAndCanonicalizePath(parts)}function createCommonjsModule(fn,module){return module={exports:{}},fn(module,module.exports),module.exports}function isFunction(x){return"function"==typeof x}function isObject(x){return null!=x&&"object"==typeof x}function tryCatcher(){try{return tryCatchTarget.apply(this,arguments)}catch(e){return errorObject_1$1.errorObject.e=e,errorObject_1$1.errorObject}}function tryCatch(fn){return tryCatchTarget=fn,tryCatcher}function toSubscriber(nextOrObserver,error,complete){if(nextOrObserver){if(nextOrObserver instanceof Subscriber_1.Subscriber)return nextOrObserver;if(nextOrObserver[rxSubscriber_1.$$rxSubscriber])return nextOrObserver[rxSubscriber_1.$$rxSubscriber]()}return nextOrObserver||error||complete?new Subscriber_1.Subscriber(nextOrObserver,error,complete):new Subscriber_1.Subscriber(Observer_1.empty)}function getSymbolObservable(context){var $$observable,Symbol=context.Symbol;return"function"==typeof Symbol?Symbol.observable?$$observable=Symbol.observable:($$observable=Symbol("observable"),Symbol.observable=$$observable):$$observable="@@observable",$$observable}function isScheduler(value){return value&&"function"==typeof value.schedule}function isPromise$2(value){return value&&"function"!=typeof value.subscribe&&"function"==typeof value.then}function symbolIteratorPonyfill(root$$1){var Symbol=root$$1.Symbol;if("function"==typeof Symbol)return Symbol.iterator||(Symbol.iterator=Symbol("iterator polyfill")),Symbol.iterator;var Set_1=root$$1.Set;if(Set_1&&"function"==typeof(new Set_1)["@@iterator"])return"@@iterator";var Map_1=root$$1.Map;if(Map_1)for(var keys=Object.getOwnPropertyNames(Map_1.prototype),i=0;i<keys.length;++i){var key=keys[i];if("entries"!==key&&"size"!==key&&Map_1.prototype[key]===Map_1.prototype.entries)return key}return"@@iterator"}function subscribeToResult(outerSubscriber,result,outerValue,outerIndex){var destination=new InnerSubscriber_1.InnerSubscriber(outerSubscriber,outerValue,outerIndex);if(destination.closed)return null;if(result instanceof Observable_1$5.Observable)return result._isScalar?(destination.next(result.value),destination.complete(),null):result.subscribe(destination);if(isArray_1$2.isArray(result)){for(var i=0,len=result.length;i<len&&!destination.closed;i++)destination.next(result[i]);destination.closed||destination.complete()}else{if(isPromise_1.isPromise(result))return result.then(function(value){destination.closed||(destination.next(value),destination.complete())},function(err){return destination.error(err)}).then(null,function(err){root_1$4.root.setTimeout(function(){throw err})}),destination;if(result&&"function"==typeof result[iterator_1.$$iterator])for(var iterator$$1=result[iterator_1.$$iterator]();;){var item=iterator$$1.next();if(item.done){destination.complete();break}if(destination.next(item.value),destination.closed)break}else if(result&&"function"==typeof result[observable_1$1.$$observable]){var obs=result[observable_1$1.$$observable]();if("function"==typeof obs.subscribe)return obs.subscribe(new InnerSubscriber_1.InnerSubscriber(outerSubscriber,outerValue,outerIndex));destination.error(new TypeError("Provided object does not correctly implement Symbol.observable"))}else{var value=isObject_1$3.isObject(result)?"an invalid object":"'"+result+"'",msg="You provided "+value+" where a stream was expected. You can provide an Observable, Promise, Array, or Iterable.";destination.error(new TypeError(msg))}}return null}function mergeAll(concurrent){return void 0===concurrent&&(concurrent=Number.POSITIVE_INFINITY),this.lift(new MergeAllOperator(concurrent))}function merge$2(){for(var observables=[],_i=0;_i<arguments.length;_i++)observables[_i-0]=arguments[_i];return this.lift.call(mergeStatic.apply(void 0,[this].concat(observables)))}function mergeStatic(){for(var observables=[],_i=0;_i<arguments.length;_i++)observables[_i-0]=arguments[_i];var concurrent=Number.POSITIVE_INFINITY,scheduler=null,last=observables[observables.length-1];return isScheduler_1.isScheduler(last)?(scheduler=observables.pop(),observables.length>1&&"number"==typeof observables[observables.length-1]&&(concurrent=observables.pop())):"number"==typeof last&&(concurrent=observables.pop()),null===scheduler&&1===observables.length?observables[0]:new ArrayObservable_1.ArrayObservable(observables,scheduler).lift(new mergeAll_1.MergeAllOperator(concurrent))}function multicast(subjectOrSubjectFactory,selector){var subjectFactory;if(subjectFactory="function"==typeof subjectOrSubjectFactory?subjectOrSubjectFactory:function(){return subjectOrSubjectFactory},"function"==typeof selector)return this.lift(new MulticastOperator(subjectFactory,selector));var connectable=Object.create(this,ConnectableObservable_1.connectableObservableDescriptor);return connectable.source=this,connectable.subjectFactory=subjectFactory,connectable}function shareSubjectFactory(){return new Subject_1.Subject}function share(){return multicast_1.multicast.call(this,shareSubjectFactory).refCount()}function makeDecorator(name,props,parentClass,chainFn){function DecoratorFactory(objOrType){if(this instanceof DecoratorFactory)return metaCtor.call(this,objOrType),this;var annotationInstance=new DecoratorFactory(objOrType),TypeDecorator=function(cls){var annotations=cls.hasOwnProperty(ANNOTATIONS)?cls[ANNOTATIONS]:Object.defineProperty(cls,ANNOTATIONS,{value:[]})[ANNOTATIONS];return annotations.push(annotationInstance),cls};return chainFn&&chainFn(TypeDecorator),TypeDecorator}var metaCtor=makeMetadataCtor(props);return parentClass&&(DecoratorFactory.prototype=Object.create(parentClass.prototype)),DecoratorFactory.prototype.ngMetadataName=name,DecoratorFactory.annotationCls=DecoratorFactory,DecoratorFactory}function makeMetadataCtor(props){return function(){for(var args=[],_i=0;_i<arguments.length;_i++)args[_i]=arguments[_i];if(props){var values=props.apply(void 0,args);for(var propName in values)this[propName]=values[propName]}}}function makeParamDecorator(name,props,parentClass){function ParamDecoratorFactory(){function ParamDecorator(cls,unusedKey,index){for(var parameters=cls.hasOwnProperty(PARAMETERS)?cls[PARAMETERS]:Object.defineProperty(cls,PARAMETERS,{value:[]})[PARAMETERS];parameters.length<=index;)parameters.push(null);return(parameters[index]=parameters[index]||[]).push(annotationInstance),cls}for(var args=[],_i=0;_i<arguments.length;_i++)args[_i]=arguments[_i];if(this instanceof ParamDecoratorFactory)return metaCtor.apply(this,args),this;var annotationInstance=new(ParamDecoratorFactory.bind.apply(ParamDecoratorFactory,[void 0].concat(args)));return ParamDecorator.annotation=annotationInstance,ParamDecorator}var metaCtor=makeMetadataCtor(props);return parentClass&&(ParamDecoratorFactory.prototype=Object.create(parentClass.prototype)),ParamDecoratorFactory.prototype.ngMetadataName=name,ParamDecoratorFactory.annotationCls=ParamDecoratorFactory,ParamDecoratorFactory}function getSymbolIterator(){if(!_symbolIterator){var Symbol=_global.Symbol;if(Symbol&&Symbol.iterator)_symbolIterator=Symbol.iterator;else for(var keys=Object.getOwnPropertyNames(Map.prototype),i=0;i<keys.length;++i){var key=keys[i];"entries"!==key&&"size"!==key&&Map.prototype[key]===Map.prototype.entries&&(_symbolIterator=key)}}return _symbolIterator}function scheduleMicroTask(fn){Zone.current.scheduleMicroTask("scheduleMicrotask",fn)}function looseIdentical(a,b){return a===b||"number"==typeof a&&"number"==typeof b&&isNaN(a)&&isNaN(b)}function stringify$1(token){if("string"==typeof token)return token;if(token instanceof Array)return"["+token.map(stringify$1).join(", ")+"]";if(null==token)return""+token;if(token.overriddenName)return""+token.overriddenName;if(token.name)return""+token.name;var res=token.toString();if(null==res)return""+res;var newLineIndex=res.indexOf("\n");return newLineIndex===-1?res:res.substring(0,newLineIndex)}function forwardRef(forwardRefFn){return forwardRefFn.__forward_ref__=forwardRef,forwardRefFn.toString=function(){return stringify$1(this())},forwardRefFn}function resolveForwardRef$1(type){return"function"==typeof type&&type.hasOwnProperty("__forward_ref__")&&type.__forward_ref__===forwardRef?type():type}function resolveProvider(provider){var deps=computeDeps(provider),fn=IDENT,value=EMPTY,useNew=!1,provide=resolveForwardRef$1(provider.provide);if(USE_VALUE$1 in provider)value=provider.useValue;else if(provider.useFactory)fn=provider.useFactory;else if(provider.useExisting);else if(provider.useClass)useNew=!0,fn=resolveForwardRef$1(provider.useClass);else{if("function"!=typeof provide)throw staticError("StaticProvider does not have [useValue|useFactory|useExisting|useClass] or [provide] is not newable",provider);useNew=!0,fn=provide}return{deps:deps,fn:fn,useNew:useNew,value:value}}function multiProviderMixError(token){return staticError("Cannot mix multi providers and regular providers",token)}function recursivelyProcessProviders(records,provider){if(provider)if(provider=resolveForwardRef$1(provider),provider instanceof Array)for(var i=0;i<provider.length;i++)recursivelyProcessProviders(records,provider[i]);else{if("function"==typeof provider)throw staticError("Function/Class not supported",provider);if(!provider||"object"!=typeof provider||!provider.provide)throw staticError("Unexpected provider",provider);var token=resolveForwardRef$1(provider.provide),resolvedProvider=resolveProvider(provider);if(provider.multi===!0){var multiProvider=records.get(token);if(multiProvider){if(multiProvider.fn!==MULTI_PROVIDER_FN)throw multiProviderMixError(token)}else records.set(token,multiProvider={token:provider.provide,deps:[],useNew:!1,fn:MULTI_PROVIDER_FN,value:EMPTY});token=provider,multiProvider.deps.push({token:token,options:6})}var record=records.get(token);if(record&&record.fn==MULTI_PROVIDER_FN)throw multiProviderMixError(token);records.set(token,resolvedProvider)}}function tryResolveToken(token,record,records,parent,notFoundValue){try{return resolveToken(token,record,records,parent,notFoundValue)}catch(e){e instanceof Error||(e=new Error(e));var path$$1=e[NG_TEMP_TOKEN_PATH]=e[NG_TEMP_TOKEN_PATH]||[];throw path$$1.unshift(token),record&&record.value==CIRCULAR&&(record.value=EMPTY),e}}function resolveToken(token,record,records,parent,notFoundValue){var value;if(record){if(value=record.value,value==CIRCULAR)throw Error(NO_NEW_LINE+"Circular dependency");if(value===EMPTY){record.value=CIRCULAR;var obj=void 0,useNew=record.useNew,fn=record.fn,depRecords=record.deps,deps=EMPTY;if(depRecords.length){deps=[];for(var i=0;i<depRecords.length;i++){var depRecord=depRecords[i],options=depRecord.options,childRecord=2&options?records.get(depRecord.token):void 0;deps.push(tryResolveToken(depRecord.token,childRecord,records,childRecord||4&options?parent:NULL_INJECTOR,1&options?null:Injector.THROW_IF_NOT_FOUND))}}record.value=value=useNew?new(fn.bind.apply(fn,[void 0].concat(deps))):fn.apply(obj,deps)}}else value=parent.get(token,notFoundValue);return value}function computeDeps(provider){var deps=EMPTY,providerDeps=provider.deps;if(providerDeps&&providerDeps.length){deps=[];for(var i=0;i<providerDeps.length;i++){var options=6,token=resolveForwardRef$1(providerDeps[i]);if(token instanceof Array)for(var j=0,annotations=token;j<annotations.length;j++){var annotation=annotations[j];annotation instanceof Optional||annotation==Optional?options=1|options:annotation instanceof SkipSelf||annotation==SkipSelf?options&=-3:annotation instanceof Self||annotation==Self?options&=-5:token=annotation instanceof Inject?annotation.token:resolveForwardRef$1(annotation)}deps.push({token:token,options:options})}}else if(provider.useExisting){var token=resolveForwardRef$1(provider.useExisting);deps=[{token:token,options:6}]}else if(!(providerDeps||USE_VALUE$1 in provider))throw staticError("'deps' required",provider);return deps}function formatError(text,obj){text=text&&"\n"===text.charAt(0)&&text.charAt(1)==NO_NEW_LINE?text.substr(2):text;var context=stringify$1(obj);if(obj instanceof Array)context=obj.map(stringify$1).join(" -> ");else if("object"==typeof obj){var parts=[];for(var key in obj)if(obj.hasOwnProperty(key)){var value=obj[key];parts.push(key+":"+("string"==typeof value?JSON.stringify(value):stringify$1(value)))}context="{"+parts.join(", ")+"}"}return"StaticInjectorError["+context+"]: "+text.replace(NEW_LINE,"\n  ")}function staticError(text,obj){return new Error(formatError(text,obj))}function getClosureSafeProperty(objWithPropertyToExtract){for(var key in objWithPropertyToExtract)if(objWithPropertyToExtract[key]===GET_PROPERTY_NAME)return key;throw Error("!prop")}function getDebugContext(error){return error[ERROR_DEBUG_CONTEXT]}function getOriginalError(error){return error[ERROR_ORIGINAL_ERROR]}function getErrorLogger(error){return error[ERROR_LOGGER]||defaultErrorLogger}function defaultErrorLogger(console){for(var values=[],_i=1;_i<arguments.length;_i++)values[_i-1]=arguments[_i];console.error.apply(console,values)}function wrappedError(message,originalError){var msg=message+" caused by: "+(originalError instanceof Error?originalError.message:originalError),error=Error(msg);return error[ERROR_ORIGINAL_ERROR]=originalError,error}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function getTemplateDiagnostics(fileName,astProvider,templates){for(var results=[],_loop_1=function(template){var ast=astProvider.getTemplateAst(template,fileName);if(ast){if(ast.parseErrors&&ast.parseErrors.length)results.push.apply(results,ast.parseErrors.map(function(e){return{kind:DiagnosticKind$$1.Error,span:offsetSpan(spanOf(e.span),template.span.start),message:e.msg}}));else if(ast.templateAst&&ast.htmlAst){var info={templateAst:ast.templateAst,htmlAst:ast.htmlAst,offset:template.span.start,query:template.query,members:template.members},expressionDiagnostics=getTemplateExpressionDiagnostics(info);results.push.apply(results,expressionDiagnostics)}ast.errors&&results.push.apply(results,ast.errors.map(function(e){return{kind:e.kind,span:e.span||template.span,message:e.message}}))}},_i=0,templates_1=templates;_i<templates_1.length;_i++){var template=templates_1[_i];_loop_1(template)}return results}function getDeclarationDiagnostics(declarations,modules){for(var results=[],directives=void 0,_loop_2=function(declaration){for(var report=function(message,span){results.push({kind:DiagnosticKind$$1.Error,span:span||declaration.declarationSpan,message:message})},_i=0,_a=declaration.errors;_i<_a.length;_i++){var error=_a[_i];report(error.message,error.span)}declaration.metadata&&(declaration.metadata.isComponent?(modules.ngModuleByPipeOrDirective.has(declaration.type)||report("Component '"+declaration.type.name+"' is not included in a module and will not be available inside a template. Consider adding it to a NgModule declaration"),declaration.metadata.template.template||declaration.metadata.template.templateUrl||report("Component "+declaration.type.name+" must have a template or templateUrl")):(directives||(directives=new Set,modules.ngModules.forEach(function(module){module.declaredDirectives.forEach(function(directive){directives.add(directive.reference)})})),directives.has(declaration.type)||report("Directive '"+declaration.type.name+"' is not included in a module and will not be available inside a template. Consider adding it to a NgModule declaration")))},_i=0,declarations_1=declarations;_i<declarations_1.length;_i++){var declaration=declarations_1[_i];_loop_2(declaration)}return results}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
Object.defineProperty(exports,"__esModule",{value:!0});var ts=require$$0__default,schema_1=schema,spreadElementSyntaxKind=ts.SyntaxKind.SpreadElement||ts.SyntaxKind.SpreadElementExpression;exports.isPrimitive=isPrimitive,exports.errorSymbol=errorSymbol;var Evaluator=function(){function Evaluator(symbols,nodeMap,options,recordExport){void 0===options&&(options={}),this.symbols=symbols,this.nodeMap=nodeMap,this.options=options,this.recordExport=recordExport}return Evaluator.prototype.nameOf=function(node){if(node&&node.kind==ts.SyntaxKind.Identifier)return node.text;var result=node&&this.evaluateNode(node);return schema_1.isMetadataError(result)||"string"==typeof result?result:errorSymbol("Name expected",node,{received:node&&node.getText()||"<missing>"})},Evaluator.prototype.isFoldable=function(node){return this.isFoldableWorker(node,new Map)},Evaluator.prototype.isFoldableWorker=function(node,folding){var _this=this;if(node)switch(node.kind){case ts.SyntaxKind.ObjectLiteralExpression:return everyNodeChild(node,function(child){if(child.kind===ts.SyntaxKind.PropertyAssignment){var propertyAssignment=child;return _this.isFoldableWorker(propertyAssignment.initializer,folding)}return!1});case ts.SyntaxKind.ArrayLiteralExpression:return everyNodeChild(node,function(child){return _this.isFoldableWorker(child,folding)});case ts.SyntaxKind.CallExpression:var callExpression=node;if(isMethodCallOf(callExpression,"concat")&&1===arrayOrEmpty(callExpression.arguments).length){var arrayNode=callExpression.expression.expression;if(this.isFoldableWorker(arrayNode,folding)&&this.isFoldableWorker(callExpression.arguments[0],folding)){var arrayValue=this.evaluateNode(arrayNode);if(arrayValue&&Array.isArray(arrayValue))return!0}}return!(!isCallOf(callExpression,"CONST_EXPR")||1!==arrayOrEmpty(callExpression.arguments).length)&&this.isFoldableWorker(callExpression.arguments[0],folding);case ts.SyntaxKind.NoSubstitutionTemplateLiteral:case ts.SyntaxKind.StringLiteral:case ts.SyntaxKind.NumericLiteral:case ts.SyntaxKind.NullKeyword:case ts.SyntaxKind.TrueKeyword:case ts.SyntaxKind.FalseKeyword:case ts.SyntaxKind.TemplateHead:case ts.SyntaxKind.TemplateMiddle:case ts.SyntaxKind.TemplateTail:return!0;case ts.SyntaxKind.ParenthesizedExpression:var parenthesizedExpression=node;return this.isFoldableWorker(parenthesizedExpression.expression,folding);case ts.SyntaxKind.BinaryExpression:var binaryExpression=node;switch(binaryExpression.operatorToken.kind){case ts.SyntaxKind.PlusToken:case ts.SyntaxKind.MinusToken:case ts.SyntaxKind.AsteriskToken:case ts.SyntaxKind.SlashToken:case ts.SyntaxKind.PercentToken:case ts.SyntaxKind.AmpersandAmpersandToken:case ts.SyntaxKind.BarBarToken:return this.isFoldableWorker(binaryExpression.left,folding)&&this.isFoldableWorker(binaryExpression.right,folding);default:return!1}case ts.SyntaxKind.PropertyAccessExpression:var propertyAccessExpression=node;return this.isFoldableWorker(propertyAccessExpression.expression,folding);case ts.SyntaxKind.ElementAccessExpression:var elementAccessExpression=node;return this.isFoldableWorker(elementAccessExpression.expression,folding)&&this.isFoldableWorker(elementAccessExpression.argumentExpression,folding);case ts.SyntaxKind.Identifier:var identifier=node,reference=this.symbols.resolve(identifier.text);if(void 0!==reference&&isPrimitive(reference))return!0;break;case ts.SyntaxKind.TemplateExpression:var templateExpression=node;return templateExpression.templateSpans.every(function(span){return _this.isFoldableWorker(span.expression,folding)})}return!1},Evaluator.prototype.evaluateNode=function(node,preferReference){function recordEntry(entry,node){if(t.options.substituteExpression){var newEntry=t.options.substituteExpression(entry,node);t.recordExport&&newEntry!=entry&&schema_1.isMetadataGlobalReferenceExpression(newEntry)&&t.recordExport(newEntry.name,entry),entry=newEntry}return t.nodeMap.set(entry,node),entry}function isFoldableError(value){return!t.options.verboseInvalidExpression&&schema_1.isMetadataError(value)}var error,_this=this,t=this,resolveName=function(name,preferReference){var reference=_this.symbols.resolve(name,preferReference);return void 0===reference?recordEntry({__symbolic:"reference",name:name},node):reference};switch(node.kind){case ts.SyntaxKind.ObjectLiteralExpression:var obj_1={},quoted_1=[];return ts.forEachChild(node,function(child){switch(child.kind){case ts.SyntaxKind.ShorthandPropertyAssignment:case ts.SyntaxKind.PropertyAssignment:var assignment=child;if(assignment.name.kind==ts.SyntaxKind.StringLiteral){var name_2=assignment.name.text;quoted_1.push(name_2)}var propertyName=_this.nameOf(assignment.name);if(isFoldableError(propertyName))return error=propertyName,!0;var propertyValue=isPropertyAssignment(assignment)?_this.evaluateNode(assignment.initializer,!0):resolveName(propertyName,!0);if(isFoldableError(propertyValue))return error=propertyValue,!0;obj_1[propertyName]=propertyValue}}),error?error:(this.options.quotedNames&&quoted_1.length&&(obj_1.$quoted$=quoted_1),recordEntry(obj_1,node));case ts.SyntaxKind.ArrayLiteralExpression:var arr_1=[];return ts.forEachChild(node,function(child){var value=_this.evaluateNode(child,!0);if(isFoldableError(value))return error=value,!0;if(schema_1.isMetadataSymbolicSpreadExpression(value)&&Array.isArray(value.expression))for(var _i=0,_a=value.expression;_i<_a.length;_i++){var spreadValue=_a[_i];arr_1.push(spreadValue)}else arr_1.push(value)}),error?error:recordEntry(arr_1,node);case spreadElementSyntaxKind:var spreadExpression=this.evaluateNode(node.expression);return recordEntry({__symbolic:"spread",expression:spreadExpression},node);case ts.SyntaxKind.CallExpression:var callExpression=node;if(isCallOf(callExpression,"forwardRef")&&1===arrayOrEmpty(callExpression.arguments).length){var firstArgument=callExpression.arguments[0];if(firstArgument.kind==ts.SyntaxKind.ArrowFunction){var arrowFunction=firstArgument;return recordEntry(this.evaluateNode(arrowFunction.body),node)}}var args=arrayOrEmpty(callExpression.arguments).map(function(arg){return _this.evaluateNode(arg)});if(!this.options.verboseInvalidExpression&&args.some(schema_1.isMetadataError))return args.find(schema_1.isMetadataError);if(this.isFoldable(callExpression)&&isMethodCallOf(callExpression,"concat")){var arrayValue=this.evaluateNode(callExpression.expression.expression);return isFoldableError(arrayValue)?arrayValue:arrayValue.concat(args[0])}if(isCallOf(callExpression,"CONST_EXPR")&&1===arrayOrEmpty(callExpression.arguments).length)return recordEntry(args[0],node);var expression=this.evaluateNode(callExpression.expression);if(isFoldableError(expression))return recordEntry(expression,node);var result={__symbolic:"call",expression:expression};return args&&args.length&&(result.arguments=args),recordEntry(result,node);case ts.SyntaxKind.NewExpression:var newExpression=node,newArgs=arrayOrEmpty(newExpression.arguments).map(function(arg){return _this.evaluateNode(arg)});if(!this.options.verboseInvalidExpression&&newArgs.some(schema_1.isMetadataError))return recordEntry(newArgs.find(schema_1.isMetadataError),node);var newTarget=this.evaluateNode(newExpression.expression);if(schema_1.isMetadataError(newTarget))return recordEntry(newTarget,node);var call={__symbolic:"new",expression:newTarget};return newArgs.length&&(call.arguments=newArgs),recordEntry(call,node);case ts.SyntaxKind.PropertyAccessExpression:var propertyAccessExpression=node,expression_1=this.evaluateNode(propertyAccessExpression.expression);if(isFoldableError(expression_1))return recordEntry(expression_1,node);var member=this.nameOf(propertyAccessExpression.name);return isFoldableError(member)?recordEntry(member,node):expression_1&&this.isFoldable(propertyAccessExpression.expression)?expression_1[member]:schema_1.isMetadataModuleReferenceExpression(expression_1)?recordEntry({__symbolic:"reference",module:expression_1.module,name:member},node):recordEntry({__symbolic:"select",expression:expression_1,member:member},node);case ts.SyntaxKind.ElementAccessExpression:var elementAccessExpression=node,expression_2=this.evaluateNode(elementAccessExpression.expression);if(isFoldableError(expression_2))return recordEntry(expression_2,node);if(!elementAccessExpression.argumentExpression)return recordEntry(errorSymbol("Expression form not supported",node),node);var index=this.evaluateNode(elementAccessExpression.argumentExpression);return isFoldableError(expression_2)?recordEntry(expression_2,node):this.isFoldable(elementAccessExpression.expression)&&this.isFoldable(elementAccessExpression.argumentExpression)?expression_2[index]:recordEntry({__symbolic:"index",expression:expression_2,index:index},node);case ts.SyntaxKind.Identifier:var identifier=node,name_3=identifier.text;return resolveName(name_3,preferReference);case ts.SyntaxKind.TypeReference:var typeReferenceNode=node,typeNameNode_1=typeReferenceNode.typeName,getReference=function(node){if(typeNameNode_1.kind===ts.SyntaxKind.QualifiedName){var qualifiedName=node,left_1=_this.evaluateNode(qualifiedName.left);return schema_1.isMetadataModuleReferenceExpression(left_1)?recordEntry({__symbolic:"reference",module:left_1.module,name:qualifiedName.right.text},node):{__symbolic:"select",expression:left_1,member:qualifiedName.right.text}}var identifier_1=typeNameNode_1,symbol=_this.symbols.resolve(identifier_1.text);return isFoldableError(symbol)||schema_1.isMetadataSymbolicReferenceExpression(symbol)?recordEntry(symbol,node):recordEntry(errorSymbol("Could not resolve type",node,{typeName:identifier_1.text}),node)},typeReference=getReference(typeNameNode_1);if(isFoldableError(typeReference))return recordEntry(typeReference,node);if(!schema_1.isMetadataModuleReferenceExpression(typeReference)&&typeReferenceNode.typeArguments&&typeReferenceNode.typeArguments.length){var args_1=typeReferenceNode.typeArguments.map(function(element){return _this.evaluateNode(element)});typeReference.arguments=args_1}return recordEntry(typeReference,node);case ts.SyntaxKind.UnionType:for(var unionType=node,references=unionType.types.filter(function(n){return n.kind!=ts.SyntaxKind.NullKeyword&&n.kind!=ts.SyntaxKind.UndefinedKeyword}).map(function(n){return _this.evaluateNode(n)}),candidate=null,i=0;i<references.length;i++){var reference=references[i];if(!schema_1.isMetadataSymbolicReferenceExpression(reference))return reference;candidate?reference.name!=candidate.name||reference.module!=candidate.module||reference.arguments||(candidate=reference):candidate=reference}if(candidate)return candidate;break;case ts.SyntaxKind.NoSubstitutionTemplateLiteral:case ts.SyntaxKind.StringLiteral:case ts.SyntaxKind.TemplateHead:case ts.SyntaxKind.TemplateTail:case ts.SyntaxKind.TemplateMiddle:return node.text;case ts.SyntaxKind.NumericLiteral:return parseFloat(node.text);case ts.SyntaxKind.AnyKeyword:return recordEntry({__symbolic:"reference",name:"any"},node);case ts.SyntaxKind.StringKeyword:return recordEntry({__symbolic:"reference",name:"string"},node);case ts.SyntaxKind.NumberKeyword:return recordEntry({__symbolic:"reference",name:"number"},node);case ts.SyntaxKind.BooleanKeyword:return recordEntry({__symbolic:"reference",name:"boolean"},node);case ts.SyntaxKind.ArrayType:var arrayTypeNode=node;return recordEntry({__symbolic:"reference",name:"Array",arguments:[this.evaluateNode(arrayTypeNode.elementType)]},node);case ts.SyntaxKind.NullKeyword:return null;case ts.SyntaxKind.TrueKeyword:return!0;case ts.SyntaxKind.FalseKeyword:return!1;case ts.SyntaxKind.ParenthesizedExpression:var parenthesizedExpression=node;return this.evaluateNode(parenthesizedExpression.expression);case ts.SyntaxKind.TypeAssertionExpression:var typeAssertion=node;return this.evaluateNode(typeAssertion.expression);case ts.SyntaxKind.PrefixUnaryExpression:var prefixUnaryExpression=node,operand=this.evaluateNode(prefixUnaryExpression.operand);if(isDefined(operand)&&isPrimitive(operand))switch(prefixUnaryExpression.operator){case ts.SyntaxKind.PlusToken:return+operand;case ts.SyntaxKind.MinusToken:return-operand;case ts.SyntaxKind.TildeToken:return~operand;case ts.SyntaxKind.ExclamationToken:return!operand}var operatorText=void 0;switch(prefixUnaryExpression.operator){case ts.SyntaxKind.PlusToken:operatorText="+";break;case ts.SyntaxKind.MinusToken:operatorText="-";break;case ts.SyntaxKind.TildeToken:operatorText="~";break;case ts.SyntaxKind.ExclamationToken:operatorText="!";break;default:return}return recordEntry({__symbolic:"pre",operator:operatorText,operand:operand},node);case ts.SyntaxKind.BinaryExpression:var binaryExpression=node,left=this.evaluateNode(binaryExpression.left),right=this.evaluateNode(binaryExpression.right);if(isDefined(left)&&isDefined(right)){if(isPrimitive(left)&&isPrimitive(right))switch(binaryExpression.operatorToken.kind){case ts.SyntaxKind.BarBarToken:return left||right;case ts.SyntaxKind.AmpersandAmpersandToken:return left&&right;case ts.SyntaxKind.AmpersandToken:return left&right;case ts.SyntaxKind.BarToken:return left|right;case ts.SyntaxKind.CaretToken:return left^right;case ts.SyntaxKind.EqualsEqualsToken:return left==right;case ts.SyntaxKind.ExclamationEqualsToken:return left!=right;case ts.SyntaxKind.EqualsEqualsEqualsToken:return left===right;case ts.SyntaxKind.ExclamationEqualsEqualsToken:return left!==right;case ts.SyntaxKind.LessThanToken:return left<right;case ts.SyntaxKind.GreaterThanToken:return left>right;case ts.SyntaxKind.LessThanEqualsToken:return left<=right;case ts.SyntaxKind.GreaterThanEqualsToken:return left>=right;case ts.SyntaxKind.LessThanLessThanToken:return left<<right;case ts.SyntaxKind.GreaterThanGreaterThanToken:return left>>right;case ts.SyntaxKind.GreaterThanGreaterThanGreaterThanToken:return left>>>right;case ts.SyntaxKind.PlusToken:return left+right;case ts.SyntaxKind.MinusToken:return left-right;case ts.SyntaxKind.AsteriskToken:return left*right;case ts.SyntaxKind.SlashToken:return left/right;case ts.SyntaxKind.PercentToken:return left%right}return recordEntry({__symbolic:"binop",operator:binaryExpression.operatorToken.getText(),left:left,right:right},node)}break;case ts.SyntaxKind.ConditionalExpression:var conditionalExpression=node,condition=this.evaluateNode(conditionalExpression.condition),thenExpression=this.evaluateNode(conditionalExpression.whenTrue),elseExpression=this.evaluateNode(conditionalExpression.whenFalse);return isPrimitive(condition)?condition?thenExpression:elseExpression:recordEntry({__symbolic:"if",condition:condition,thenExpression:thenExpression,elseExpression:elseExpression},node);case ts.SyntaxKind.FunctionExpression:case ts.SyntaxKind.ArrowFunction:return recordEntry(errorSymbol("Function call not supported",node),node);case ts.SyntaxKind.TaggedTemplateExpression:return recordEntry(errorSymbol("Tagged template expressions are not supported in metadata",node),node);case ts.SyntaxKind.TemplateExpression:var templateExpression=node;return this.isFoldable(node)?templateExpression.templateSpans.reduce(function(previous,current){return previous+_this.evaluateNode(current.expression)+_this.evaluateNode(current.literal)},this.evaluateNode(templateExpression.head)):templateExpression.templateSpans.reduce(function(previous,current){var expr=_this.evaluateNode(current.expression),literal=_this.evaluateNode(current.literal);if(isFoldableError(expr))return expr;if(isFoldableError(literal))return literal;if("string"==typeof previous&&"string"==typeof expr&&"string"==typeof literal)return previous+expr+literal;var result=expr;return""!==previous&&(result={__symbolic:"binop",operator:"+",left:previous,right:expr}),""!=literal&&(result={__symbolic:"binop",operator:"+",left:result,right:literal}),result},this.evaluateNode(templateExpression.head));case ts.SyntaxKind.AsExpression:var asExpression=node;return this.evaluateNode(asExpression.expression);case ts.SyntaxKind.ClassExpression:return{__symbolic:"class"}}return recordEntry(errorSymbol("Expression form not supported",node),node)},Evaluator}();exports.Evaluator=Evaluator;var empty=ts.createNodeArray()}),symbols=createCommonjsModule(function(module,exports){function populateBuiltins(symbols){["Object","Function","String","Number","Array","Boolean","Map","NaN","Infinity","Math","Date","RegExp","Error","Error","EvalError","RangeError","ReferenceError","SyntaxError","TypeError","URIError","JSON","ArrayBuffer","DataView","Int8Array","Uint8Array","Uint8ClampedArray","Uint16Array","Int16Array","Int32Array","Uint32Array","Float32Array","Float64Array"].forEach(function(name){return symbols.set(name,{__symbolic:"reference",name:name})})}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var __extends=commonjsGlobal&&commonjsGlobal.__extends||function(){var extendStatics=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(d,b){d.__proto__=b}||function(d,b){for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p])};return function(d,b){function __(){this.constructor=d}extendStatics(d,b),d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)}}();Object.defineProperty(exports,"__esModule",{value:!0});var compiler_1=require$$1$1,tsc_wrapped_1=collector,fs$$1=fs__default,path$$1=path__default,ts=require$$0__default,EXT=/(\.ts|\.d\.ts|\.js|\.jsx|\.tsx)$/,DTS=/\.d\.ts$/,NODE_MODULES="/node_modules/",IS_GENERATED=/\.(ngfactory|ngstyle|ngsummary)$/,GENERATED_FILES=/\.ngfactory\.ts$|\.ngstyle\.ts$|\.ngsummary\.ts$/,GENERATED_OR_DTS_FILES=/\.d\.ts$|\.ngfactory\.ts$|\.ngstyle\.ts$|\.ngsummary\.ts$/,SHALLOW_IMPORT=/^((\w|-)+|(@(\w|-)+(\/(\w|-)+)+))$/,BaseAotCompilerHost=function(){function BaseAotCompilerHost(program,options,context,metadataProvider){void 0===metadataProvider&&(metadataProvider=new tsc_wrapped_1.MetadataCollector),this.program=program,this.options=options,this.context=context,this.metadataProvider=metadataProvider,this.resolverCache=new Map,this.flatModuleIndexCache=new Map,this.flatModuleIndexNames=new Set,this.flatModuleIndexRedirectNames=new Set}return BaseAotCompilerHost.prototype.getSourceFile=function(filePath){var sf=this.program.getSourceFile(filePath);if(!sf){if(this.context.fileExists(filePath)){var sourceText=this.context.readFile(filePath);return ts.createSourceFile(filePath,sourceText,ts.ScriptTarget.Latest,!0)}throw new Error("Source file "+filePath+" not present in program.")}return sf},BaseAotCompilerHost.prototype.getMetadataFor=function(filePath){if(this.context.fileExists(filePath)){if(DTS.test(filePath)){var metadataPath=filePath.replace(DTS,".metadata.json");return this.context.fileExists(metadataPath)?this.readMetadata(metadataPath,filePath):[this.upgradeVersion1Metadata({__symbolic:"module",version:1,metadata:{}},filePath)]}var sf=this.getSourceFile(filePath),metadata=this.metadataProvider.getMetadata(sf);return metadata?[metadata]:[]}},BaseAotCompilerHost.prototype.readMetadata=function(filePath,dtsFilePath){var metadatas=this.resolverCache.get(filePath);if(metadatas)return metadatas;try{var metadataOrMetadatas=JSON.parse(this.context.readFile(filePath)),metadatas_1=metadataOrMetadatas?Array.isArray(metadataOrMetadatas)?metadataOrMetadatas:[metadataOrMetadatas]:[],v1Metadata=metadatas_1.find(function(m){return 1===m.version}),v3Metadata=metadatas_1.find(function(m){return 3===m.version});return!v3Metadata&&v1Metadata&&metadatas_1.push(this.upgradeVersion1Metadata(v1Metadata,dtsFilePath)),this.resolverCache.set(filePath,metadatas_1),metadatas_1}catch(e){throw console.error("Failed to read JSON file "+filePath),e}},BaseAotCompilerHost.prototype.upgradeVersion1Metadata=function(v1Metadata,dtsFilePath){var v3Metadata={__symbolic:"module",version:3,metadata:{}};v1Metadata.exports&&(v3Metadata.exports=v1Metadata.exports);for(var prop in v1Metadata.metadata)v3Metadata.metadata[prop]=v1Metadata.metadata[prop];var exports=this.metadataProvider.getMetadata(this.getSourceFile(dtsFilePath));if(exports){for(var prop in exports.metadata)v3Metadata.metadata[prop]||(v3Metadata.metadata[prop]=exports.metadata[prop]);exports.exports&&(v3Metadata.exports=exports.exports)}return v3Metadata},BaseAotCompilerHost.prototype.loadResource=function(filePath){if(this.context.readResource)return this.context.readResource(filePath);if(!this.context.fileExists(filePath))throw compiler_1.syntaxError("Error: Resource file not found: "+filePath);return this.context.readFile(filePath)},BaseAotCompilerHost.prototype.loadSummary=function(filePath){return this.context.fileExists(filePath)?this.context.readFile(filePath):null},BaseAotCompilerHost.prototype.isSourceFile=function(filePath){var excludeRegex=this.options.generateCodeForLibraries===!1?GENERATED_OR_DTS_FILES:GENERATED_FILES;if(excludeRegex.test(filePath))return!1;if(DTS.test(filePath)&&this.hasBundleIndex(filePath)){var normalFilePath=path$$1.normalize(filePath);return this.flatModuleIndexNames.has(normalFilePath)||this.flatModuleIndexRedirectNames.has(normalFilePath)}return!0},BaseAotCompilerHost.prototype.hasBundleIndex=function(filePath){var _this=this,checkBundleIndex=function(directory){var result=_this.flatModuleIndexCache.get(directory);if(null==result){if("node_module"==path$$1.basename(directory))result=!1;else try{var packageFile=path$$1.join(directory,"package.json");if(_this.context.fileExists(packageFile)){result=!1;var packageContent=JSON.parse(_this.context.readFile(packageFile));if(packageContent.typings){var typings=path$$1.normalize(path$$1.join(directory,packageContent.typings));if(DTS.test(typings)){var metadataFile=typings.replace(DTS,".metadata.json");if(_this.context.fileExists(metadataFile)){var metadata=JSON.parse(_this.context.readFile(metadataFile));metadata.flatModuleIndexRedirect?_this.flatModuleIndexRedirectNames.add(typings):metadata.importAs&&(_this.flatModuleIndexNames.add(typings),result=!0)}}}}else{var parent_1=path$$1.dirname(directory);result=parent_1!=directory&&checkBundleIndex(parent_1)}}catch(e){result=!1}_this.flatModuleIndexCache.set(directory,result)}return result};return checkBundleIndex(path$$1.dirname(filePath))},BaseAotCompilerHost}();exports.BaseAotCompilerHost=BaseAotCompilerHost;var CompilerHost=function(_super){function CompilerHost(program,options,context,collectorOptions,metadataProvider){void 0===metadataProvider&&(metadataProvider=new tsc_wrapped_1.MetadataCollector(collectorOptions));var _this=_super.call(this,program,options,context,metadataProvider)||this;_this.moduleFileNames=new Map,_this.basePath=path$$1.normalize(path$$1.join(_this.options.basePath,".")).replace(/\\/g,"/"),_this.genDir=path$$1.normalize(path$$1.join(_this.options.genDir,".")).replace(/\\/g,"/");var genPath=path$$1.relative(_this.basePath,_this.genDir);return _this.isGenDirChildOfRootDir=""===genPath||!genPath.startsWith(".."),_this.resolveModuleNameHost=Object.create(_this.context),_this.resolveModuleNameHost.fileExists=function(fileName){if(_this.context.fileExists(fileName))return!0;if(DTS.test(fileName)){var base=fileName.substring(0,fileName.length-5);return _this.context.fileExists(base+".ngsummary.json")}return!1},_this.urlResolver=compiler_1.createOfflineCompileUrlResolver(),_this}return __extends(CompilerHost,_super),CompilerHost.prototype.toSummaryFileName=function(fileName,referringSrcFileName){return fileName.replace(EXT,"")+".d.ts"},CompilerHost.prototype.fromSummaryFileName=function(fileName,referringLibFileName){return fileName},CompilerHost.prototype.calculateEmitPath=function(filePath){for(var root=this.options.basePath,_i=0,_a=this.options.rootDirs||[];_i<_a.length;_i++){var eachRootDir=_a[_i];this.options.trace&&console.error("Check if "+filePath+" is under rootDirs element "+eachRootDir),0!==path$$1.relative(eachRootDir,filePath).indexOf(".")&&(root=eachRootDir)}for(var relativePath=path$$1.relative(root,filePath);relativePath.startsWith(".."+path$$1.sep);)relativePath=relativePath.substr(3);return path$$1.join(this.options.genDir,relativePath)},CompilerHost.prototype.getCanonicalFileName=function(fileName){return fileName},CompilerHost.prototype.moduleNameToFileName=function(m,containingFile){var key=m+":"+(containingFile||""),result=this.moduleFileNames.get(key)||null;if(!result){if(!containingFile||!containingFile.length){if(0===m.indexOf("."))throw new Error("Resolution of relative paths requires a containing file.");containingFile=this.getCanonicalFileName(path$$1.join(this.basePath,"index.ts"))}m=m.replace(EXT,"");var resolved=ts.resolveModuleName(m,containingFile.replace(/\\/g,"/"),this.options,this.resolveModuleNameHost).resolvedModule;result=resolved?this.getCanonicalFileName(resolved.resolvedFileName):null,this.moduleFileNames.set(key,result)}return result},CompilerHost.prototype.fileNameToModuleName=function(importedFile,containingFile){importedFile===containingFile||this.context.fileExists(importedFile)||this.context.assumeFileExists(importedFile),containingFile=this.rewriteGenDirPath(containingFile);var containingDir=path$$1.dirname(containingFile);importedFile=importedFile.replace(EXT,"");var nodeModulesIndex=importedFile.indexOf(NODE_MODULES),importModule=nodeModulesIndex===-1?null:importedFile.substring(nodeModulesIndex+NODE_MODULES.length),isGeneratedFile=IS_GENERATED.test(importedFile);return isGeneratedFile?importModule?this.dotRelative(containingDir,this.genDir+NODE_MODULES+importModule):(importedFile=this.rewriteGenDirPath(importedFile),this.dotRelative(containingDir,importedFile)):importModule?importModule:(this.isGenDirChildOfRootDir||(importedFile=importedFile.replace(this.basePath,this.genDir)),SHALLOW_IMPORT.test(importedFile)?importedFile:this.dotRelative(containingDir,importedFile))},CompilerHost.prototype.resourceNameToFileName=function(m,containingFile){return this.urlResolver.resolve(containingFile,m)},CompilerHost.prototype.rewriteGenDirPath=function(filepath){var nodeModulesIndex=filepath.indexOf(NODE_MODULES);return nodeModulesIndex!==-1?path$$1.join(this.genDir,filepath.substring(nodeModulesIndex)):filepath.replace(this.basePath,this.genDir)},CompilerHost.prototype.dotRelative=function(from,to){var rPath=path$$1.relative(from,to).replace(/\\/g,"/");return rPath.startsWith(".")?rPath:"./"+rPath},CompilerHost}(BaseAotCompilerHost);exports.CompilerHost=CompilerHost;var CompilerHostContextAdapter=function(){function CompilerHostContextAdapter(){this.assumedExists={}}return CompilerHostContextAdapter.prototype.assumeFileExists=function(fileName){this.assumedExists[fileName]=!0},CompilerHostContextAdapter}();exports.CompilerHostContextAdapter=CompilerHostContextAdapter;var ModuleResolutionHostAdapter=function(_super){function ModuleResolutionHostAdapter(host){var _this=_super.call(this)||this;return _this.host=host,host.directoryExists&&(_this.directoryExists=function(directoryName){return host.directoryExists(directoryName)}),_this}return __extends(ModuleResolutionHostAdapter,_super),ModuleResolutionHostAdapter.prototype.fileExists=function(fileName){return this.assumedExists[fileName]||this.host.fileExists(fileName)},ModuleResolutionHostAdapter.prototype.readFile=function(fileName){return this.host.readFile(fileName)},ModuleResolutionHostAdapter.prototype.readResource=function(s){if(!this.host.fileExists(s))throw new Error("Compilation failed. Resource file not found: "+s);return Promise.resolve(this.host.readFile(s))},ModuleResolutionHostAdapter}(CompilerHostContextAdapter);exports.ModuleResolutionHostAdapter=ModuleResolutionHostAdapter;var NodeCompilerHostContext=function(_super){function NodeCompilerHostContext(){return null!==_super&&_super.apply(this,arguments)||this}return __extends(NodeCompilerHostContext,_super),NodeCompilerHostContext.prototype.fileExists=function(fileName){return this.assumedExists[fileName]||fs$$1.existsSync(fileName)},NodeCompilerHostContext.prototype.directoryExists=function(directoryName){try{return fs$$1.statSync(directoryName).isDirectory()}catch(e){return!1}},NodeCompilerHostContext.prototype.readFile=function(fileName){return fs$$1.readFileSync(fileName,"utf8")},NodeCompilerHostContext.prototype.readResource=function(s){if(!this.fileExists(s))throw new Error("Compilation failed. Resource file not found: "+s);return Promise.resolve(this.readFile(s))},NodeCompilerHostContext}(CompilerHostContextAdapter);exports.NodeCompilerHostContext=NodeCompilerHostContext}),check_types=createCommonjsModule(function(module,exports){function diagnosticMessageToString(message){return ts.flattenDiagnosticMessageText(message,"\n")}function createFactoryInfo(emitter,file){var _a=emitter.emitStatementsAndContext(file.srcFileUrl,file.genFileUrl,file.stmts),sourceText=_a.sourceText,context=_a.context,source=ts.createSourceFile(file.genFileUrl,sourceText,ts.ScriptTarget.Latest,!0);return{source:source,context:context}}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
Object.defineProperty(exports,"__esModule",{value:!0});var compiler_1=require$$1$1,ts=require$$0__default,stubCancellationToken={isCancellationRequested:function(){return!1},throwIfCancellationRequested:function(){}},TypeChecker=function(){function TypeChecker(program,tsOptions,compilerHost,aotCompilerHost,aotOptions,_analyzedModules,_generatedFiles){this.program=program,this.tsOptions=tsOptions,this.compilerHost=compilerHost,this.aotCompilerHost=aotCompilerHost,this.aotOptions=aotOptions,this._analyzedModules=_analyzedModules,this._generatedFiles=_generatedFiles,this._currentCancellationToken=stubCancellationToken,this._partial=!1}return TypeChecker.prototype.getDiagnostics=function(fileName,cancellationToken){this._currentCancellationToken=cancellationToken||stubCancellationToken;try{return fileName?this.diagnosticsByFileName.get(fileName)||[]:(_a=[]).concat.apply(_a,Array.from(this.diagnosticsByFileName.values()))}finally{this._currentCancellationToken=stubCancellationToken}var _a},Object.defineProperty(TypeChecker.prototype,"partialResults",{get:function(){return this._partial},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"analyzedModules",{get:function(){return this._analyzedModules||(this._analyzedModules=this.aotCompiler.analyzeModulesSync(this.program.getSourceFiles().map(function(sf){return sf.fileName})))},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"diagnosticsByFileName",{get:function(){return this._diagnosticsByFile||this.createDiagnosticsByFile()},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"diagnosticProgram",{get:function(){return this._diagnosticProgram||this.createDiagnosticProgram()},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"generatedFiles",{get:function(){var result=this._generatedFiles;return result||(this._generatedFiles=result=this.aotCompiler.emitAllImpls(this.analyzedModules)),result},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"aotCompiler",{get:function(){return this._aotCompiler||this.createCompilerAndReflector()},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"reflector",{get:function(){var result=this._reflector;return result||(this.createCompilerAndReflector(),result=this._reflector),result},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"factories",{get:function(){return this._factories||this.createFactories()},enumerable:!0,configurable:!0}),Object.defineProperty(TypeChecker.prototype,"factoryNames",{get:function(){return this._factoryNames||this.createFactories()&&this._factoryNames},enumerable:!0,configurable:!0}),TypeChecker.prototype.createCompilerAndReflector=function(){var _a=compiler_1.createAotCompiler(this.aotCompilerHost,this.aotOptions),compiler=_a.compiler,reflector=_a.reflector;return this._reflector=reflector,this._aotCompiler=compiler},TypeChecker.prototype.createDiagnosticProgram=function(){var existingFiles=this.program.getSourceFiles().map(function(source){return source.fileName}),host=new TypeCheckingHost(this.compilerHost,this.program,this.factories);return this._diagnosticProgram=ts.createProgram(existingFiles.concat(this.factoryNames),this.tsOptions,host)},TypeChecker.prototype.createFactories=function(){var emitter=new compiler_1.TypeScriptEmitter,factorySources=this.generatedFiles.filter(function(file){return null!=file.stmts&&file.stmts.length}).map(function(file){return[file.genFileUrl,createFactoryInfo(emitter,file)]});return this._factories=new Map(factorySources),this._factoryNames=Array.from(this._factories.keys()),this._factories},TypeChecker.prototype.createDiagnosticsByFile=function(){for(var result=new Map,diagnosticsFor=function(fileName){var r=result.get(fileName);return r||(r=[],result.set(fileName,r)),r},program=this.diagnosticProgram,_i=0,_a=this.factoryNames;_i<_a.length;_i++){var factoryName=_a[_i];if(this._currentCancellationToken.isCancellationRequested())return result;for(var sourceFile=program.getSourceFile(factoryName),_b=0,_c=this.diagnosticProgram.getSemanticDiagnostics(sourceFile);_b<_c.length;_b++){var diagnostic=_c[_b],span=this.sourceSpanOf(diagnostic.file,diagnostic.start,diagnostic.length);if(span){var fileName=span.start.file.url,diagnosticsList=diagnosticsFor(fileName);diagnosticsList.push({message:diagnosticMessageToString(diagnostic.messageText),category:diagnostic.category,span:span})}}}return result},TypeChecker.prototype.sourceSpanOf=function(source,start,length){var info=this.factories.get(source.fileName);if(info){var _a=ts.getLineAndCharacterOfPosition(source,start),line=_a.line,character=_a.character;return info.context.spanOf(line,character)}return null},TypeChecker}();exports.TypeChecker=TypeChecker;var TypeCheckingHost=function(){function TypeCheckingHost(host,originalProgram,factories){this.host=host,this.originalProgram=originalProgram,this.factories=factories,this.writeFile=function(){throw new Error("Unexpected write in diagnostic program")}}return TypeCheckingHost.prototype.getSourceFile=function(fileName,languageVersion,onError){var originalSource=this.originalProgram.getSourceFile(fileName);if(originalSource)return originalSource;var factoryInfo=this.factories.get(fileName);return factoryInfo?factoryInfo.source:this.host.getSourceFile(fileName,languageVersion,onError)},TypeCheckingHost.prototype.getDefaultLibFileName=function(options){return this.host.getDefaultLibFileName(options)},TypeCheckingHost.prototype.getCurrentDirectory=function(){return this.host.getCurrentDirectory()},TypeCheckingHost.prototype.getDirectories=function(path$$1){return this.host.getDirectories(path$$1)},TypeCheckingHost.prototype.getCanonicalFileName=function(fileName){return this.host.getCanonicalFileName(fileName)},TypeCheckingHost.prototype.useCaseSensitiveFileNames=function(){return this.host.useCaseSensitiveFileNames()},TypeCheckingHost.prototype.getNewLine=function(){return this.host.getNewLine()},TypeCheckingHost.prototype.fileExists=function(fileName){return this.factories.has(fileName)||this.host.fileExists(fileName)},TypeCheckingHost.prototype.readFile=function(fileName){var factoryInfo=this.factories.get(fileName);return factoryInfo&&factoryInfo.source.text||this.host.readFile(fileName)},TypeCheckingHost}()}),symbols$2=createCommonjsModule(function(module,exports){/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
Object.defineProperty(exports,"__esModule",{value:!0});var BuiltinType;!function(BuiltinType){BuiltinType[BuiltinType.Any=0]="Any",BuiltinType[BuiltinType.String=1]="String",BuiltinType[BuiltinType.Number=2]="Number",BuiltinType[BuiltinType.Boolean=3]="Boolean",BuiltinType[BuiltinType.Undefined=4]="Undefined",BuiltinType[BuiltinType.Null=5]="Null",BuiltinType[BuiltinType.Unbound=6]="Unbound",BuiltinType[BuiltinType.Other=7]="Other"}(BuiltinType=exports.BuiltinType||(exports.BuiltinType={}))}),expression_type=createCommonjsModule(function(module,exports){/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
Object.defineProperty(exports,"__esModule",{value:!0});var DiagnosticKind,compiler_1=require$$1$1,symbols_1=symbols$2;!function(DiagnosticKind){DiagnosticKind[DiagnosticKind.Error=0]="Error",DiagnosticKind[DiagnosticKind.Warning=1]="Warning"}(DiagnosticKind=exports.DiagnosticKind||(exports.DiagnosticKind={}));var TypeDiagnostic=function(){function TypeDiagnostic(kind,message,ast){this.kind=kind,this.message=message,this.ast=ast}return TypeDiagnostic}();exports.TypeDiagnostic=TypeDiagnostic;var AstType=function(){function AstType(scope,query,context){this.scope=scope,this.query=query,this.context=context}return AstType.prototype.getType=function(ast){return ast.visit(this)},AstType.prototype.getDiagnostics=function(ast){this.diagnostics=[];var type=ast.visit(this);return this.context.event&&type.callable&&this.reportWarning("Unexpected callable expression. Expected a method call",ast),this.diagnostics},AstType.prototype.visitBinary=function(ast){function normalize(kind,other){switch(kind){case symbols_1.BuiltinType.Undefined:case symbols_1.BuiltinType.Null:return normalize(other,symbols_1.BuiltinType.Other)}return kind}var _this=this,getType=function(ast,operation){var type=_this.getType(ast);if(type.nullable){switch(operation){case"&&":case"||":case"==":case"!=":case"===":case"!==":break;default:_this.reportError("The expression might be null",ast)}return _this.query.getNonNullableType(type)}return type},leftType=getType(ast.left,ast.operation),rightType=getType(ast.right,ast.operation),leftRawKind=this.query.getTypeKind(leftType),rightRawKind=this.query.getTypeKind(rightType),leftKind=normalize(leftRawKind,rightRawKind),rightKind=normalize(rightRawKind,leftRawKind),operKind=leftKind<<8|rightKind;switch(ast.operation){case"*":case"/":case"%":case"-":case"<<":case">>":case">>>":case"&":case"^":case"|":switch(operKind){case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Number:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Number:return this.query.getBuiltinType(symbols_1.BuiltinType.Number);default:var errorAst=ast.left;switch(leftKind){case symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Number:errorAst=ast.right}return this.reportError("Expected a numeric type",errorAst)}case"+":switch(operKind){case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Boolean:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Number:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Other:case symbols_1.BuiltinType.Boolean<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Other<<8|symbols_1.BuiltinType.Any:return this.anyType;case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.String:case symbols_1.BuiltinType.Boolean<<8|symbols_1.BuiltinType.String:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.String:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.Boolean:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.Number:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.String:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.Other:case symbols_1.BuiltinType.Other<<8|symbols_1.BuiltinType.String:return this.query.getBuiltinType(symbols_1.BuiltinType.String);case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Number:return this.query.getBuiltinType(symbols_1.BuiltinType.Number);case symbols_1.BuiltinType.Boolean<<8|symbols_1.BuiltinType.Number:case symbols_1.BuiltinType.Other<<8|symbols_1.BuiltinType.Number:return this.reportError("Expected a number type",ast.left);case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Boolean:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Other:return this.reportError("Expected a number type",ast.right);default:return this.reportError("Expected operands to be a string or number type",ast)}case">":case"<":case"<=":case">=":case"==":case"!=":case"===":case"!==":switch(operKind){case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Boolean:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Number:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.String:case symbols_1.BuiltinType.Any<<8|symbols_1.BuiltinType.Other:case symbols_1.BuiltinType.Boolean<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Boolean<<8|symbols_1.BuiltinType.Boolean:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Number<<8|symbols_1.BuiltinType.Number:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.String<<8|symbols_1.BuiltinType.String:case symbols_1.BuiltinType.Other<<8|symbols_1.BuiltinType.Any:case symbols_1.BuiltinType.Other<<8|symbols_1.BuiltinType.Other:return this.query.getBuiltinType(symbols_1.BuiltinType.Boolean);default:return this.reportError("Expected the operants to be of similar type or any",ast)}case"&&":return rightType;case"||":return this.query.getTypeUnion(leftType,rightType)}return this.reportError("Unrecognized operator "+ast.operation,ast)},AstType.prototype.visitChain=function(ast){return this.diagnostics&&compiler_1.visitAstChildren(ast,this),this.query.getBuiltinType(symbols_1.BuiltinType.Undefined)},AstType.prototype.visitConditional=function(ast){return this.diagnostics&&compiler_1.visitAstChildren(ast,this),this.query.getTypeUnion(this.getType(ast.trueExp),this.getType(ast.falseExp))},AstType.prototype.visitFunctionCall=function(ast){var _this=this,args=ast.args.map(function(arg){return _this.getType(arg)}),target=this.getType(ast.target);if(!target||!target.callable)return this.reportError("Call target is not callable",ast);var signature=target.selectSignature(args);return signature?signature.result:this.reportError("Unable no compatible signature found for call",ast)},AstType.prototype.visitImplicitReceiver=function(ast){var _this=this;return{name:"$implict",kind:"component",language:"ng-template",type:void 0,container:void 0,callable:!1,nullable:!1,public:!0,definition:void 0,members:function(){return _this.scope},signatures:function(){return[]},selectSignature:function(types){},indexed:function(argument){}}},AstType.prototype.visitInterpolation=function(ast){return this.diagnostics&&compiler_1.visitAstChildren(ast,this),this.undefinedType},AstType.prototype.visitKeyedRead=function(ast){var targetType=this.getType(ast.obj),keyType=this.getType(ast.key),result=targetType.indexed(keyType);return result||this.anyType},AstType.prototype.visitKeyedWrite=function(ast){return this.getType(ast.value)},AstType.prototype.visitLiteralArray=function(ast){var _this=this;return this.query.getArrayType((_a=this.query).getTypeUnion.apply(_a,ast.expressions.map(function(element){return _this.getType(element)})));var _a},AstType.prototype.visitLiteralMap=function(ast){return this.diagnostics&&compiler_1.visitAstChildren(ast,this),this.anyType},AstType.prototype.visitLiteralPrimitive=function(ast){switch(ast.value){case!0:case!1:return this.query.getBuiltinType(symbols_1.BuiltinType.Boolean);case null:return this.query.getBuiltinType(symbols_1.BuiltinType.Null);case void 0:return this.query.getBuiltinType(symbols_1.BuiltinType.Undefined);default:switch(typeof ast.value){case"string":return this.query.getBuiltinType(symbols_1.BuiltinType.String);case"number":return this.query.getBuiltinType(symbols_1.BuiltinType.Number);default:return this.reportError("Unrecognized primitive",ast)}}},AstType.prototype.visitMethodCall=function(ast){return this.resolveMethodCall(this.getType(ast.receiver),ast)},AstType.prototype.visitPipe=function(ast){var _this=this,pipe=this.query.getPipes().get(ast.name);if(!pipe)return this.reportError("No pipe by the name "+ast.name+" found",ast);var expType=this.getType(ast.exp),signature=pipe.selectSignature([expType].concat(ast.args.map(function(arg){return _this.getType(arg)})));return signature?signature.result:this.reportError("Unable to resolve signature for pipe invocation",ast)},AstType.prototype.visitPrefixNot=function(ast){return this.query.getBuiltinType(symbols_1.BuiltinType.Boolean)},AstType.prototype.visitNonNullAssert=function(ast){var expressionType=this.getType(ast.expression);return this.query.getNonNullableType(expressionType)},AstType.prototype.visitPropertyRead=function(ast){return this.resolvePropertyRead(this.getType(ast.receiver),ast)},AstType.prototype.visitPropertyWrite=function(ast){return this.getType(ast.value)},AstType.prototype.visitQuote=function(ast){return this.query.getBuiltinType(symbols_1.BuiltinType.Any)},AstType.prototype.visitSafeMethodCall=function(ast){return this.resolveMethodCall(this.query.getNonNullableType(this.getType(ast.receiver)),ast)},AstType.prototype.visitSafePropertyRead=function(ast){return this.resolvePropertyRead(this.query.getNonNullableType(this.getType(ast.receiver)),ast)},Object.defineProperty(AstType.prototype,"anyType",{get:function(){var result=this._anyType;return result||(result=this._anyType=this.query.getBuiltinType(symbols_1.BuiltinType.Any)),result},enumerable:!0,configurable:!0}),Object.defineProperty(AstType.prototype,"undefinedType",{get:function(){var result=this._undefinedType;return result||(result=this._undefinedType=this.query.getBuiltinType(symbols_1.BuiltinType.Undefined)),result},enumerable:!0,configurable:!0}),AstType.prototype.resolveMethodCall=function(receiverType,ast){var _this=this;if(this.isAny(receiverType))return this.anyType;var method=receiverType.members().get(ast.name);if(!method)return this.reportError("Unknown method '"+ast.name+"'",ast);if(!method.type)return this.reportError("Could not find a type for '"+ast.name+"'",ast);if(!method.type.callable)return this.reportError("Member '"+ast.name+"' is not callable",ast);var signature=method.type.selectSignature(ast.args.map(function(arg){return _this.getType(arg)}));return signature?signature.result:this.reportError("Unable to resolve signature for call of method "+ast.name,ast)},AstType.prototype.resolvePropertyRead=function(receiverType,ast){if(this.isAny(receiverType))return this.anyType;var member=receiverType.members().get(ast.name);if(!member){var receiverInfo=receiverType.name;if("$implict"==receiverInfo)receiverInfo="The component declaration, template variable declarations, and element references do";else{if(receiverType.nullable)return this.reportError("The expression might be null",ast.receiver);receiverInfo="'"+receiverInfo+"' does"}return this.reportError("Identifier '"+ast.name+"' is not defined. "+receiverInfo+" not contain such a member",ast)}if(!member.public){var receiverInfo=receiverType.name;receiverInfo="$implict"==receiverInfo?"the component":"'"+receiverInfo+"'",this.reportWarning("Identifier '"+ast.name+"' refers to a private member of "+receiverInfo,ast)}return member.type},AstType.prototype.reportError=function(message,ast){return this.diagnostics&&this.diagnostics.push(new TypeDiagnostic(DiagnosticKind.Error,message,ast)),this.anyType},AstType.prototype.reportWarning=function(message,ast){return this.diagnostics&&this.diagnostics.push(new TypeDiagnostic(DiagnosticKind.Warning,message,ast)),this.anyType},AstType.prototype.isAny=function(symbol){return!symbol||this.query.getTypeKind(symbol)==symbols_1.BuiltinType.Any||!!symbol.type&&this.isAny(symbol.type)},AstType}();exports.AstType=AstType}),expression_diagnostics=createCommonjsModule(function(module,exports){function getTemplateExpressionDiagnostics(info){var visitor=new ExpressionDiagnosticsVisitor(info,function(path$$1,includeEvent){return getExpressionScope(info,path$$1,includeEvent)});return compiler_1.templateVisitAll(visitor,info.templateAst),visitor.diagnostics}function getExpressionDiagnostics(scope,ast,query,context){void 0===context&&(context={});var analyzer=new expression_type_1.AstType(scope,query,context);return analyzer.getDiagnostics(ast),analyzer.diagnostics}function getReferences(info){function processReferences(references){for(var _loop_1=function(reference){var type=void 0;reference.value&&(type=info.query.getTypeSymbol(compiler_1.tokenReference(reference.value))),result.push({name:reference.name,kind:"reference",type:type||info.query.getBuiltinType(symbols_1.BuiltinType.Any),get definition(){return getDefinitionOf(info,reference)}})},_i=0,references_1=references;_i<references_1.length;_i++){var reference=references_1[_i];_loop_1(reference)}}var result=[],visitor=new(function(_super){function class_1(){return null!==_super&&_super.apply(this,arguments)||this}return __extends(class_1,_super),class_1.prototype.visitEmbeddedTemplate=function(ast,context){_super.prototype.visitEmbeddedTemplate.call(this,ast,context),processReferences(ast.references)},class_1.prototype.visitElement=function(ast,context){_super.prototype.visitElement.call(this,ast,context),processReferences(ast.references)},class_1}(compiler_1.RecursiveTemplateAstVisitor));return compiler_1.templateVisitAll(visitor,info.templateAst),result}function getDefinitionOf(info,ast){if(info.fileName){var templateOffset=info.offset;return[{fileName:info.fileName,span:{start:ast.sourceSpan.start.offset+templateOffset,end:ast.sourceSpan.end.offset+templateOffset}}]}}function getVarDeclarations(info,path$$1){for(var result=[],current=path$$1.tail;current;){if(current instanceof compiler_1.EmbeddedTemplateAst)for(var _loop_2=function(variable){var name_1=variable.name,context=current.directives.map(function(d){return info.query.getTemplateContext(d.directive.type.reference)}).find(function(c){return!!c}),type=void 0;if(context){var value=context.get(variable.value);if(value){type=value.type;var kind=info.query.getTypeKind(type);kind!==symbols_1.BuiltinType.Any&&kind!=symbols_1.BuiltinType.Unbound||(type=refinedVariableType(type,info,current))}}type||(type=info.query.getBuiltinType(symbols_1.BuiltinType.Any)),result.push({name:name_1,kind:"variable",type:type,get definition(){return getDefinitionOf(info,variable)}})},_i=0,_a=current.variables;_i<_a.length;_i++){var variable=_a[_i];_loop_2(variable)}current=path$$1.parentOf(current)}return result}function refinedVariableType(type,info,templateElement){var ngForDirective=templateElement.directives.find(function(d){var name=compiler_1.identifierName(d.directive.type);return"NgFor"==name||"NgForOf"==name});if(ngForDirective){var ngForOfBinding=ngForDirective.inputs.find(function(i){return"ngForOf"==i.directiveName});if(ngForOfBinding){var bindingType=new expression_type_1.AstType(info.members,info.query,{}).getType(ngForOfBinding.value);if(bindingType){var result=info.query.getElementType(bindingType);if(result)return result}}}return info.query.getBuiltinType(symbols_1.BuiltinType.Any)}function getEventDeclaration(info,includeEvent){var result=[];return includeEvent&&(result=[{name:"$event",kind:"variable",type:info.query.getBuiltinType(symbols_1.BuiltinType.Any)}]),result}function getExpressionScope(info,path$$1,includeEvent){var result=info.members,references=getReferences(info),variables=getVarDeclarations(info,path$$1),events=getEventDeclaration(info,includeEvent);if(references.length||variables.length||events.length){var referenceTable=info.query.createSymbolTable(references),variableTable=info.query.createSymbolTable(variables),eventsTable=info.query.createSymbolTable(events);result=info.query.mergeSymbolTable([result,referenceTable,variableTable,eventsTable])}return result}function hasTemplateReference(type){if(type.diDeps)for(var _i=0,_a=type.diDeps;_i<_a.length;_i++){var diDep=_a[_i];if(diDep.token&&diDep.token.identifier&&"TemplateRef"==compiler_1.identifierName(diDep.token.identifier))return!0}return!1}function offsetSpan(span,amount){return{start:span.start+amount,end:span.end+amount}}function spanOf(sourceSpan){return{start:sourceSpan.start.offset,end:sourceSpan.end.offset}}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
var __extends=commonjsGlobal&&commonjsGlobal.__extends||function(){var extendStatics=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(d,b){d.__proto__=b}||function(d,b){for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p])};return function(d,b){function __(){this.constructor=d}extendStatics(d,b),d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)}}();Object.defineProperty(exports,"__esModule",{value:!0});var compiler_1=require$$1$1,expression_type_1=expression_type,symbols_1=symbols$2;exports.getTemplateExpressionDiagnostics=getTemplateExpressionDiagnostics,exports.getExpressionDiagnostics=getExpressionDiagnostics,exports.getExpressionScope=getExpressionScope;var ExpressionDiagnosticsVisitor=function(_super){function ExpressionDiagnosticsVisitor(info,getExpressionScope){var _this=_super.call(this)||this;return _this.info=info,_this.getExpressionScope=getExpressionScope,_this.diagnostics=[],_this.path=new compiler_1.AstPath([]),_this}return __extends(ExpressionDiagnosticsVisitor,_super),ExpressionDiagnosticsVisitor.prototype.visitDirective=function(ast,context){ast.inputs&&ast.inputs.length&&compiler_1.templateVisitAll(this,ast.inputs,context)},ExpressionDiagnosticsVisitor.prototype.visitBoundText=function(ast){this.push(ast),this.diagnoseExpression(ast.value,ast.sourceSpan.start.offset,!1),this.pop()},ExpressionDiagnosticsVisitor.prototype.visitDirectiveProperty=function(ast){this.push(ast),this.diagnoseExpression(ast.value,this.attributeValueLocation(ast),!1),this.pop()},ExpressionDiagnosticsVisitor.prototype.visitElementProperty=function(ast){this.push(ast),this.diagnoseExpression(ast.value,this.attributeValueLocation(ast),!1),this.pop()},ExpressionDiagnosticsVisitor.prototype.visitEvent=function(ast){this.push(ast),this.diagnoseExpression(ast.handler,this.attributeValueLocation(ast),!0),this.pop()},ExpressionDiagnosticsVisitor.prototype.visitVariable=function(ast){var directive=this.directiveSummary;if(directive&&ast.value){var context=this.info.query.getTemplateContext(directive.type.reference);context&&!context.has(ast.value)&&("$implicit"===ast.value?this.reportError("The template context does not have an implicit value",spanOf(ast.sourceSpan)):this.reportError("The template context does not defined a member called '"+ast.value+"'",spanOf(ast.sourceSpan)))}},ExpressionDiagnosticsVisitor.prototype.visitElement=function(ast,context){this.push(ast),_super.prototype.visitElement.call(this,ast,context),this.pop()},ExpressionDiagnosticsVisitor.prototype.visitEmbeddedTemplate=function(ast,context){var previousDirectiveSummary=this.directiveSummary;this.push(ast),this.directiveSummary=ast.directives.map(function(d){return d.directive}).find(function(d){return hasTemplateReference(d.type)}),_super.prototype.visitEmbeddedTemplate.call(this,ast,context),this.pop(),this.directiveSummary=previousDirectiveSummary},ExpressionDiagnosticsVisitor.prototype.attributeValueLocation=function(ast){var path$$1=compiler_1.findNode(this.info.htmlAst,ast.sourceSpan.start.offset),last=path$$1.tail;return last instanceof compiler_1.Attribute&&last.valueSpan?last.valueSpan.start.offset+1:ast.sourceSpan.start.offset},ExpressionDiagnosticsVisitor.prototype.diagnoseExpression=function(ast,offset,includeEvent){var _this=this,scope=this.getExpressionScope(this.path,includeEvent);(_a=this.diagnostics).push.apply(_a,getExpressionDiagnostics(scope,ast,this.info.query,{event:includeEvent}).map(function(d){return{span:offsetSpan(d.ast.span,offset+_this.info.offset),kind:d.kind,message:d.message}}));var _a},ExpressionDiagnosticsVisitor.prototype.push=function(ast){this.path.push(ast)},ExpressionDiagnosticsVisitor.prototype.pop=function(){this.path.pop()},ExpressionDiagnosticsVisitor.prototype.reportError=function(message,span){span&&this.diagnostics.push({span:offsetSpan(span,this.info.offset),kind:expression_type_1.DiagnosticKind.Error,message:message})},ExpressionDiagnosticsVisitor.prototype.reportWarning=function(message,span){this.diagnostics.push({span:offsetSpan(span,this.info.offset),kind:expression_type_1.DiagnosticKind.Warning,message:message})},ExpressionDiagnosticsVisitor}(compiler_1.RecursiveTemplateAstVisitor)}),typescript_symbols=createCommonjsModule(function(module,exports){function getSymbolQuery(program,checker,source,fetchPipes){return new TypeScriptSymbolQuery(program,checker,source,fetchPipes)}function getClassMembers(program,checker,staticSymbol){var declaration=getClassFromStaticSymbol(program,staticSymbol);if(declaration){var type=checker.getTypeAtLocation(declaration),node=program.getSourceFile(staticSymbol.filePath);return new TypeWrapper(type,{node:node,program:program,checker:checker}).members()}}function getClassMembersFromDeclaration(program,checker,source,declaration){var type=checker.getTypeAtLocation(declaration);return new TypeWrapper(type,{node:source,program:program,checker:checker}).members()}function getClassFromStaticSymbol(program,type){var source=program.getSourceFile(type.filePath);if(source)return ts.forEachChild(source,function(child){if(child.kind===ts.SyntaxKind.ClassDeclaration){var classDeclaration=child;if(null!=classDeclaration.name&&classDeclaration.name.text===type.name)return classDeclaration}})}function getPipesTable(source,program,checker,pipes){return new PipesTable(pipes,{program:program,checker:checker,node:source})}function typeCallable(type){var signatures=type.getCallSignatures();return signatures&&0!=signatures.length}function signaturesOf(type,context){return type.getCallSignatures().map(function(s){return new SignatureWrapper(s,context)})}function selectSignature(type,context,types){var signatures=type.getCallSignatures();return signatures.length?new SignatureWrapper(signatures[0],context):void 0}function toSymbols(symbolTable){if(!symbolTable)return[];var table=symbolTable;if("function"==typeof table.values)return Array.from(table.values());var result=[],own="function"==typeof table.hasOwnProperty?function(name){return table.hasOwnProperty(name)}:function(name){return!!table[name]};for(var name_1 in table)own(name_1)&&result.push(table[name_1]);return result}function findClassSymbolInContext(type,context){var sourceFile=context.program.getSourceFile(type.filePath);if(sourceFile){var moduleSymbol=sourceFile.module||sourceFile.symbol,exports_1=context.checker.getExportsOfModule(moduleSymbol);return(exports_1||[]).find(function(symbol){return symbol.name==type.name})}}function isSymbolPrivate(s){return!!s.valueDeclaration&&isPrivate(s.valueDeclaration)}function getBuiltinTypeFromTs(kind,context){var type,checker=context.checker,node=context.node;switch(kind){case symbols_1.BuiltinType.Any:type=checker.getTypeAtLocation(setParents({kind:ts.SyntaxKind.AsExpression,expression:{kind:ts.SyntaxKind.TrueKeyword},type:{kind:ts.SyntaxKind.AnyKeyword}},node));break;case symbols_1.BuiltinType.Boolean:type=checker.getTypeAtLocation(setParents({kind:ts.SyntaxKind.TrueKeyword},node));break;case symbols_1.BuiltinType.Null:type=checker.getTypeAtLocation(setParents({kind:ts.SyntaxKind.NullKeyword},node));break;case symbols_1.BuiltinType.Number:var numeric={kind:ts.SyntaxKind.NumericLiteral};setParents({kind:ts.SyntaxKind.ExpressionStatement,expression:numeric},node),type=checker.getTypeAtLocation(numeric);break;case symbols_1.BuiltinType.String:type=checker.getTypeAtLocation(setParents({kind:ts.SyntaxKind.NoSubstitutionTemplateLiteral},node));break;case symbols_1.BuiltinType.Undefined:type=checker.getTypeAtLocation(setParents({kind:ts.SyntaxKind.VoidExpression,expression:{kind:ts.SyntaxKind.NumericLiteral}},node));break;default:throw new Error("Internal error, unhandled literal kind "+kind+":"+symbols_1.BuiltinType[kind])}return type}function setParents(node,parent){return node.parent=parent,ts.forEachChild(node,function(child){return setParents(child,node)}),node}function spanAt(sourceFile,line,column){if(null!=line&&null!=column){var position_1=ts.getPositionOfLineAndCharacter(sourceFile,line,column),findChild=function findChild(node){if(node.kind>ts.SyntaxKind.LastToken&&node.pos<=position_1&&node.end>position_1){var betterNode=ts.forEachChild(node,findChild);return betterNode||node}},node=ts.forEachChild(sourceFile,findChild);if(node)return{start:node.getStart(),end:node.getEnd()}}}function definitionFromTsSymbol(symbol){var declarations=symbol.declarations;if(declarations)return declarations.map(function(declaration){var sourceFile=declaration.getSourceFile();return{fileName:sourceFile.fileName,span:{start:declaration.getStart(),end:declaration.getEnd()}}})}function parentDeclarationOf(node){for(;node;){switch(node.kind){case ts.SyntaxKind.ClassDeclaration:case ts.SyntaxKind.InterfaceDeclaration:return node;case ts.SyntaxKind.SourceFile:return}node=node.parent}}function getContainerOf(symbol,context){if(symbol.getFlags()&ts.SymbolFlags.ClassMember&&symbol.declarations)for(var _i=0,_a=symbol.declarations;_i<_a.length;_i++){var declaration=_a[_i],parent_1=parentDeclarationOf(declaration);if(parent_1){var type=context.checker.getTypeAtLocation(parent_1);if(type)return new TypeWrapper(type,context)}}}function getTypeParameterOf(type,name){if(type&&type.symbol&&type.symbol.name==name){var typeArguments=type.typeArguments;if(typeArguments&&typeArguments.length<=1)return typeArguments[0]}}function typeKindOf(type){if(type){if(type.flags&ts.TypeFlags.Any)return symbols_1.BuiltinType.Any;if(type.flags&(ts.TypeFlags.String|ts.TypeFlags.StringLike|ts.TypeFlags.StringLiteral))return symbols_1.BuiltinType.String;if(type.flags&(ts.TypeFlags.Number|ts.TypeFlags.NumberLike))return symbols_1.BuiltinType.Number;if(type.flags&ts.TypeFlags.Undefined)return symbols_1.BuiltinType.Undefined;if(type.flags&ts.TypeFlags.Null)return symbols_1.BuiltinType.Null;if(type.flags&ts.TypeFlags.Union){var candidate=null,unionType=type;if(unionType.types.length>0){candidate=typeKindOf(unionType.types[0]);for(var _i=0,_a=unionType.types;_i<_a.length;_i++){var subType=_a[_i];if(candidate!=typeKindOf(subType))return symbols_1.BuiltinType.Other}}if(null!=candidate)return candidate}else if(type.flags&ts.TypeFlags.TypeParameter)return symbols_1.BuiltinType.Unbound}return symbols_1.BuiltinType.Other}function getFromSymbolTable(symbolTable,key){var symbol,table=symbolTable;return symbol="function"==typeof table.get?table.get(key):table[key]}function toNumbers(value){return value?value.split(".").map(function(v){return+v}):[]}function compareNumbers(a,b){for(var i=0;i<a.length&&i<b.length;i++){if(a[i]>b[i])return 1;if(a[i]<b[i])return-1}return 0}function isTypescriptVersion(low,high){var tsNumbers=toNumbers(ts.version);return compareNumbers(toNumbers(low),tsNumbers)<=0&&compareNumbers(toNumbers(high),tsNumbers)>=0}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
Object.defineProperty(exports,"__esModule",{value:!0});var ts=require$$0__default,symbols_1=symbols$2,isPrivate=ts.ModifierFlags?function(node){return!!(ts.getCombinedModifierFlags(node)&ts.ModifierFlags.Private)}:function(node){return!!(node.flags&ts.NodeFlags.Private)},isReferenceType=ts.ObjectFlags?function(type){return!!(type.flags&ts.TypeFlags.Object&&type.objectFlags&ts.ObjectFlags.Reference)}:function(type){return!!(type.flags&ts.TypeFlags.Reference)};exports.getSymbolQuery=getSymbolQuery,exports.getClassMembers=getClassMembers,exports.getClassMembersFromDeclaration=getClassMembersFromDeclaration,exports.getClassFromStaticSymbol=getClassFromStaticSymbol,exports.getPipesTable=getPipesTable;var TypeScriptSymbolQuery=function(){function TypeScriptSymbolQuery(program,checker,source,fetchPipes){this.program=program,this.checker=checker,this.source=source,this.fetchPipes=fetchPipes,this.typeCache=new Map}return TypeScriptSymbolQuery.prototype.getTypeKind=function(symbol){return typeKindOf(this.getTsTypeOf(symbol))},TypeScriptSymbolQuery.prototype.getBuiltinType=function(kind){var result=this.typeCache.get(kind);if(!result){var type=getBuiltinTypeFromTs(kind,{checker:this.checker,node:this.source,program:this.program});result=new TypeWrapper(type,{program:this.program,checker:this.checker,node:this.source}),this.typeCache.set(kind,result)}return result},TypeScriptSymbolQuery.prototype.getTypeUnion=function(){for(var types=[],_i=0;_i<arguments.length;_i++)types[_i]=arguments[_i];var result=void 0;if(types.length){result=types[0];for(var i=1;i<types.length;i++)if(types[i]!=result){result=void 0;break}}return result||this.getBuiltinType(symbols_1.BuiltinType.Any)},TypeScriptSymbolQuery.prototype.getArrayType=function(type){return this.getBuiltinType(symbols_1.BuiltinType.Any)},TypeScriptSymbolQuery.prototype.getElementType=function(type){if(type instanceof TypeWrapper){var elementType=getTypeParameterOf(type.tsType,"Array");if(elementType)return new TypeWrapper(elementType,type.context)}},TypeScriptSymbolQuery.prototype.getNonNullableType=function(symbol){if(symbol instanceof TypeWrapper&&"function"==typeof this.checker.getNonNullableType){var tsType=symbol.tsType,nonNullableType=this.checker.getNonNullableType(tsType);if(nonNullableType!=tsType)return new TypeWrapper(nonNullableType,symbol.context);if(nonNullableType==tsType)return symbol}return this.getBuiltinType(symbols_1.BuiltinType.Any)},TypeScriptSymbolQuery.prototype.getPipes=function(){var result=this.pipesCache;return result||(result=this.pipesCache=this.fetchPipes()),result},TypeScriptSymbolQuery.prototype.getTemplateContext=function(type){var context={node:this.source,program:this.program,checker:this.checker},typeSymbol=findClassSymbolInContext(type,context);if(typeSymbol){var contextType=this.getTemplateRefContextType(typeSymbol);if(contextType)return new SymbolWrapper(contextType,context).members()}},TypeScriptSymbolQuery.prototype.getTypeSymbol=function(type){var context={node:this.source,program:this.program,checker:this.checker},typeSymbol=findClassSymbolInContext(type,context);return typeSymbol&&new SymbolWrapper(typeSymbol,context)},TypeScriptSymbolQuery.prototype.createSymbolTable=function(symbols){var result=new MapSymbolTable;return result.addAll(symbols.map(function(s){return new DeclaredSymbol(s)})),result},TypeScriptSymbolQuery.prototype.mergeSymbolTable=function(symbolTables){for(var result=new MapSymbolTable,_i=0,symbolTables_1=symbolTables;_i<symbolTables_1.length;_i++){var symbolTable=symbolTables_1[_i];result.addAll(symbolTable.values())}return result},TypeScriptSymbolQuery.prototype.getSpanAt=function(line,column){return spanAt(this.source,line,column)},TypeScriptSymbolQuery.prototype.getTemplateRefContextType=function(typeSymbol){var type=this.checker.getTypeOfSymbolAtLocation(typeSymbol,this.source),constructor=type.symbol&&type.symbol.members&&getFromSymbolTable(type.symbol.members,"__constructor");if(constructor)for(var constructorDeclaration=constructor.declarations[0],_i=0,_a=constructorDeclaration.parameters;_i<_a.length;_i++){var parameter=_a[_i],type_1=this.checker.getTypeAtLocation(parameter.type);if("TemplateRef"==type_1.symbol.name&&isReferenceType(type_1)){var typeReference=type_1;if(1===typeReference.typeArguments.length)return typeReference.typeArguments[0].symbol}}},TypeScriptSymbolQuery.prototype.getTsTypeOf=function(symbol){var type=this.getTypeWrapper(symbol);return type&&type.tsType},TypeScriptSymbolQuery.prototype.getTypeWrapper=function(symbol){var type=void 0;return symbol instanceof TypeWrapper?type=symbol:symbol.type instanceof TypeWrapper&&(type=symbol.type),type},TypeScriptSymbolQuery}(),TypeWrapper=function(){function TypeWrapper(tsType,context){if(this.tsType=tsType,this.context=context,!tsType)throw Error("Internal: null type")}return Object.defineProperty(TypeWrapper.prototype,"name",{get:function(){var symbol=this.tsType.symbol;return symbol&&symbol.name||"<anonymous>"},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"kind",{get:function(){return"type"},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"language",{get:function(){return"typescript"},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"type",{get:function(){},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"container",{get:function(){},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"public",{get:function(){return!0},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"callable",{get:function(){return typeCallable(this.tsType)},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"nullable",{get:function(){return this.context.checker.getNonNullableType(this.tsType)!=this.tsType},enumerable:!0,configurable:!0}),Object.defineProperty(TypeWrapper.prototype,"definition",{get:function(){return definitionFromTsSymbol(this.tsType.getSymbol())},enumerable:!0,configurable:!0}),TypeWrapper.prototype.members=function(){return new SymbolTableWrapper(this.tsType.getProperties(),this.context)},TypeWrapper.prototype.signatures=function(){return signaturesOf(this.tsType,this.context)},TypeWrapper.prototype.selectSignature=function(types){return selectSignature(this.tsType,this.context,types)},TypeWrapper.prototype.indexed=function(argument){},TypeWrapper}(),SymbolWrapper=function(){function SymbolWrapper(symbol,context){this.context=context,this.symbol=symbol&&context&&symbol.flags&ts.SymbolFlags.Alias?context.checker.getAliasedSymbol(symbol):symbol}return Object.defineProperty(SymbolWrapper.prototype,"name",{get:function(){return this.symbol.name},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"kind",{get:function(){return this.callable?"method":"property"},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"language",{get:function(){return"typescript"},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"type",{get:function(){return new TypeWrapper(this.tsType,this.context)},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"container",{get:function(){return getContainerOf(this.symbol,this.context)},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"public",{get:function(){return!isSymbolPrivate(this.symbol)},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"callable",{get:function(){return typeCallable(this.tsType)},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"nullable",{get:function(){return!1},enumerable:!0,configurable:!0}),Object.defineProperty(SymbolWrapper.prototype,"definition",{get:function(){return definitionFromTsSymbol(this.symbol)},enumerable:!0,configurable:!0}),SymbolWrapper.prototype.members=function(){if(!this._members)if(0!=(this.symbol.flags&(ts.SymbolFlags.Class|ts.SymbolFlags.Interface))){var declaredType=this.context.checker.getDeclaredTypeOfSymbol(this.symbol),typeWrapper=new TypeWrapper(declaredType,this.context);this._members=typeWrapper.members()}else this._members=new SymbolTableWrapper(this.symbol.members,this.context);return this._members},SymbolWrapper.prototype.signatures=function(){return signaturesOf(this.tsType,this.context)},SymbolWrapper.prototype.selectSignature=function(types){return selectSignature(this.tsType,this.context,types)},SymbolWrapper.prototype.indexed=function(argument){},Object.defineProperty(SymbolWrapper.prototype,"tsType",{get:function(){var type=this._tsType;return type||(type=this._tsType=this.context.checker.getTypeOfSymbolAtLocation(this.symbol,this.context.node)),type},enumerable:!0,configurable:!0}),SymbolWrapper}(),DeclaredSymbol=function(){function DeclaredSymbol(declaration){this.declaration=declaration}return Object.defineProperty(DeclaredSymbol.prototype,"name",{get:function(){return this.declaration.name},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"kind",{get:function(){return this.declaration.kind},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"language",{get:function(){return"ng-template"},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"container",{get:function(){},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"type",{get:function(){return this.declaration.type},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"callable",{get:function(){return this.declaration.type.callable},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"nullable",{get:function(){return!1},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"public",{get:function(){return!0},enumerable:!0,configurable:!0}),Object.defineProperty(DeclaredSymbol.prototype,"definition",{get:function(){return this.declaration.definition},enumerable:!0,configurable:!0}),DeclaredSymbol.prototype.members=function(){return this.declaration.type.members()},DeclaredSymbol.prototype.signatures=function(){return this.declaration.type.signatures()},DeclaredSymbol.prototype.selectSignature=function(types){return this.declaration.type.selectSignature(types)},DeclaredSymbol.prototype.indexed=function(argument){},DeclaredSymbol}(),SignatureWrapper=function(){function SignatureWrapper(signature,context){this.signature=signature,this.context=context}return Object.defineProperty(SignatureWrapper.prototype,"arguments",{get:function(){return new SymbolTableWrapper(this.signature.getParameters(),this.context)},enumerable:!0,configurable:!0}),Object.defineProperty(SignatureWrapper.prototype,"result",{get:function(){return new TypeWrapper(this.signature.getReturnType(),this.context)},enumerable:!0,configurable:!0}),SignatureWrapper}(),SignatureResultOverride=function(){function SignatureResultOverride(signature,resultType){this.signature=signature,this.resultType=resultType}return Object.defineProperty(SignatureResultOverride.prototype,"arguments",{get:function(){return this.signature.arguments},enumerable:!0,configurable:!0}),Object.defineProperty(SignatureResultOverride.prototype,"result",{get:function(){return this.resultType},enumerable:!0,configurable:!0}),SignatureResultOverride}(),toSymbolTable=isTypescriptVersion("2.2")?function(symbols){for(var result=new Map,_i=0,symbols_2=symbols;_i<symbols_2.length;_i++){var symbol=symbols_2[_i];result.set(symbol.name,symbol)}return result}:function(symbols){for(var result={},_i=0,symbols_3=symbols;_i<symbols_3.length;_i++){var symbol=symbols_3[_i];result[symbol.name]=symbol}return result},SymbolTableWrapper=function(){function SymbolTableWrapper(symbols,context){this.context=context,symbols=symbols||[],Array.isArray(symbols)?(this.symbols=symbols,this.symbolTable=toSymbolTable(symbols)):(this.symbols=toSymbols(symbols),this.symbolTable=symbols)}return Object.defineProperty(SymbolTableWrapper.prototype,"size",{get:function(){return this.symbols.length},enumerable:!0,configurable:!0}),SymbolTableWrapper.prototype.get=function(key){var symbol=getFromSymbolTable(this.symbolTable,key);return symbol?new SymbolWrapper(symbol,this.context):void 0},SymbolTableWrapper.prototype.has=function(key){var table=this.symbolTable;return"function"==typeof table.has?table.has(key):null!=table[key]},SymbolTableWrapper.prototype.values=function(){var _this=this;return this.symbols.map(function(s){return new SymbolWrapper(s,_this.context)})},SymbolTableWrapper}(),MapSymbolTable=function(){function MapSymbolTable(){this.map=new Map,this._values=[]}return Object.defineProperty(MapSymbolTable.prototype,"size",{get:function(){return this.map.size},enumerable:!0,configurable:!0}),MapSymbolTable.prototype.get=function(key){return this.map.get(key)},MapSymbolTable.prototype.add=function(symbol){if(this.map.has(symbol.name)){var previous=this.map.get(symbol.name);this._values[this._values.indexOf(previous)]=symbol}this.map.set(symbol.name,symbol),this._values.push(symbol)},MapSymbolTable.prototype.addAll=function(symbols){for(var _i=0,symbols_4=symbols;_i<symbols_4.length;_i++){var symbol=symbols_4[_i];this.add(symbol)}},MapSymbolTable.prototype.has=function(key){return this.map.has(key)},MapSymbolTable.prototype.values=function(){return this._values},MapSymbolTable}(),PipesTable=function(){function PipesTable(pipes,context){this.pipes=pipes,this.context=context}return Object.defineProperty(PipesTable.prototype,"size",{get:function(){return this.pipes.length},enumerable:!0,configurable:!0}),PipesTable.prototype.get=function(key){var pipe=this.pipes.find(function(pipe){return pipe.name==key});if(pipe)return new PipeSymbol(pipe,this.context)},PipesTable.prototype.has=function(key){return null!=this.pipes.find(function(pipe){return pipe.name==key})},PipesTable.prototype.values=function(){var _this=this;return this.pipes.map(function(pipe){return new PipeSymbol(pipe,_this.context)})},PipesTable}(),PipeSymbol=function(){function PipeSymbol(pipe,context){this.pipe=pipe,this.context=context}return Object.defineProperty(PipeSymbol.prototype,"name",{get:function(){return this.pipe.name},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"kind",{get:function(){return"pipe"},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"language",{get:function(){return"typescript"},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"type",{get:function(){return new TypeWrapper(this.tsType,this.context)},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"container",{get:function(){},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"callable",{get:function(){return!0},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"nullable",{get:function(){return!1},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"public",{get:function(){return!0},enumerable:!0,configurable:!0}),Object.defineProperty(PipeSymbol.prototype,"definition",{get:function(){return definitionFromTsSymbol(this.tsType.getSymbol())},enumerable:!0,configurable:!0}),PipeSymbol.prototype.members=function(){return EmptyTable.instance},PipeSymbol.prototype.signatures=function(){return signaturesOf(this.tsType,this.context)},PipeSymbol.prototype.selectSignature=function(types){var signature=selectSignature(this.tsType,this.context,types);if(1==types.length){var parameterType=types[0];if(parameterType instanceof TypeWrapper){var resultType=void 0;switch(this.name){case"async":switch(parameterType.name){case"Observable":case"Promise":case"EventEmitter":resultType=getTypeParameterOf(parameterType.tsType,parameterType.name);break;default:resultType=getBuiltinTypeFromTs(symbols_1.BuiltinType.Any,this.context)}break;case"slice":resultType=getTypeParameterOf(parameterType.tsType,"Array")}resultType&&(signature=new SignatureResultOverride(signature,new TypeWrapper(resultType,parameterType.context)))}}return signature},PipeSymbol.prototype.indexed=function(argument){},Object.defineProperty(PipeSymbol.prototype,"tsType",{get:function(){var type=this._tsType;if(!type){var classSymbol=this.findClassSymbol(this.pipe.type.reference);classSymbol&&(type=this._tsType=this.findTransformMethodType(classSymbol)),type||(type=this._tsType=getBuiltinTypeFromTs(symbols_1.BuiltinType.Any,this.context))}return type},enumerable:!0,configurable:!0}),PipeSymbol.prototype.findClassSymbol=function(type){return findClassSymbolInContext(type,this.context)},PipeSymbol.prototype.findTransformMethodType=function(classSymbol){var classType=this.context.checker.getDeclaredTypeOfSymbol(classSymbol);if(classType){var transform=classType.getProperty("transform");if(transform)return this.context.checker.getTypeOfSymbolAtLocation(transform,this.context.node)}},PipeSymbol}(),EmptyTable=function(){function EmptyTable(){}return Object.defineProperty(EmptyTable.prototype,"size",{get:function(){return 0},enumerable:!0,configurable:!0}),EmptyTable.prototype.get=function(key){},EmptyTable.prototype.has=function(key){return!1},EmptyTable.prototype.values=function(){return[]},EmptyTable}();EmptyTable.instance=new EmptyTable}),language_services=createCommonjsModule(function(module,exports){/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var projectHostMap=new WeakMap,angularOnlyResults=process.argv.indexOf("--angularOnlyResults")>=0,VERSION$$1=new Version$1("5.0.0-beta.4-97c69744f");exports.createLanguageService=createLanguageService,exports.TypeScriptServiceHost=TypeScriptServiceHost,exports.createLanguageServiceFromTypescript=createLanguageServiceFromTypescript,exports.VERSION=VERSION$$1,exports.getExternalFiles=getExternalFiles,exports.create=create,Object.defineProperty(exports,"__esModule",{value:!0})});
//# sourceMappingURL=/Users/cedric/Code/angular/angular/dist/packages-dist/language-service/bundles/language-service.umd.min.js.map